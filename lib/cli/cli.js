...
const onShutdown = require('../utilities/will-interrupt-process');
  ...
  run(environment) {
    return RSVP.hash(environment).then(environment => {
      ...
      let runPromise = Promise.resolve().then(() => {
        // try {
        removeShutdownHandler = onShutdown.addHandler(onCommandInterrupt);
        //     .addHandler(() => shutdown = command.onInterrupt());
        // } catch (e) {
        //   console.error(e);
        //   throw e;
        // }
        ...
        console.log('cli: command.beforeRun');
        loggerTesting.info('cli: command.beforeRun');

        return command.beforeRun(commandArgs);
        // return waitShutdown(command.beforeRun(commandArgs), shutdown);
      }).then(() => {
        console.log('cli: command.validateAndRun');
        loggerTesting.info('cli: command.validateAndRun');

        // return waitShutdown(command.validateAndRun(commandArgs), shutdown);
        return command.validateAndRun(commandArgs);
      }).finally(() => {
        console.log('cli: finally')
        try {
          if (shutdown) {
            shutdown.finally(removeShutdownHandler);
          } else {
        ...
        console.log(`cli: command run complete. exitCode: ${exitCode}`);
...
function waitShutdown(work, shutdown) {
  console.log('waitForShutdown: shutdown', shutdown);
  shutdown = Promise.resolve(shutdown);
